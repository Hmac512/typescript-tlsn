...
export const encode = (
  opening: CommitmentOpeningInterface,
  encodings: EncodingInterface[]
) => {
  if (encodings.length != opening.Blake3.data.length)
    throw new Error('encodings and data must have the same length');

  const resp: LeafInterface[] = [];
  ...
  for (let i = 0; i < encodings.length; i++) {
    ...
    const labels = [];
    for (let j = 0; j < 8; j++) {
      const label = new BN(encoding.labels[j]);
      label.xor;
      ...
      // console.log("val", val)
      const encoded = val.toArray("be", 16);
